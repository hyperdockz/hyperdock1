...
vm_check_image_lock() {
    ...
    if lsof "$img_file" 2>/dev/null | grep -q qemu-system; then
        ...
        if [ -n "$pid" ]; then
            ...
            if ps -p "$pid" -o cmd= | grep -q "$vm_name"; then
                vm_print_status INFO "This appears to be the same VM"
                local kill_choice
                vm_read_char kill_choice "Kill existing process and restart? (y/N): "
                if [[ "$kill_choice" =~ ^[Yy]$ ]]; then
                    kill "$pid" && sleep 2 && kill -0 "$pid" 2>/dev/null && kill -9 "$pid"
                    return 0
                fi
            else
                vm_print_status ERROR "Another QEMU instance is using this image"
            fi
        fi
        return 1
    fi
    ...
    if [ -f "$lock_file" ]; then
        ...
        if find "$lock_file" -mmin +5 2>/dev/null | read -r; then
            vm_print_status WARN "Lock file is older than 5 minutes"
            local remove_lock
            vm_read_char remove_lock "Remove stale lock file? (y/N): "
...
vm_validate_input() {
    ...
    case "$type" in
        number)
            if ! [[ "$value" =~ ^[0-9]+$ ]]; then
                vm_print_status ERROR "Must be a number"
                return 1
            fi
            ;;
        size)
            if ! [[ "$value" =~ ^[0-9]+[GgMm]$ ]]; then
                vm_print_status ERROR "Must be a size with unit (e.g., 100G, 512M)"
                return 1
            fi
            ;;
        port)
            if ! [[ "$value" =~ ^[0-9]+$ ]] || [ "$value" -lt 1024 ] || [ "$value" -gt 65535 ]; then
                vm_print_status ERROR "Must be a port number between 1024-65535"
                return 1
            fi
            ;;
        name)
            if ! [[ "$value" =~ ^[a-zA-Z0-9_-]+$ ]]; then
                vm_print_status ERROR "Name can only contain letters, numbers, hyphens, and underscores"
                return 1
            fi
            ;;
        username)
            if ! [[ "$value" =~ ^[a-z_][a-z0-9_-]*$ ]]; then
                vm_print_status ERROR "Username must start with a letter or underscore"
                return 1
            fi
            ;;
    esac
...
vm_setup_image() {
    ...
    mkdir -p "$VM_DIR"
    if [ -f "$IMG_FILE" ]; then
        vm_print_status INFO "Image file already exists. Skipping download."
    else
        vm_print_status INFO "Downloading image from $IMG_URL..."
        if ! wget --progress=bar:force "$IMG_URL" -O "$IMG_FILE.tmp"; then
            vm_print_status ERROR "Failed to download image"
            return 1
        fi
...
EOF
    if ! cloud-localds "$SEED_FILE" user-data meta-data; then
        vm_print_status ERROR "Failed to create cloud-init seed image"
        return 1
    fi
    return 0
}
...
vm_create_new() {
    ...
    local i=1
    local os_options=()
    vm_print_status INFO "Select an OS to set up:"
    ...
    while true; do
        ...
        if vm_validate_input "name" "$VM_NAME" && [ ! -f "$VM_DIR/$VM_NAME.conf" ]; then
            break
        elif [ -f "$VM_DIR/$VM_NAME.conf" ]; then
            vm_print_status ERROR "VM with name '$VM_NAME' already exists"
        fi
    done
    while true; do vm_read HOSTNAME "Hostname (default: $VM_NAME): "; HOSTNAME="${HOSTNAME:-$VM_NAME}"; vm_validate_input "name" "$HOSTNAME" && break; done
    while true; do vm_read USERNAME "Username (default: $DEFAULT_USERNAME): "; USERNAME="${USERNAME:-$DEFAULT_USERNAME}"; vm_validate_input "username" "$USERNAME" && break; done
    while true; do vm_read_secret PASSWORD "Password (default: $DEFAULT_PASSWORD): "; PASSWORD="${PASSWORD:-$DEFAULT_PASSWORD}"; [ -n "$PASSWORD" ] && break || vm_print_status ERROR "Password cannot be empty"; done
    while true; do vm_read DISK_SIZE "Disk size (default: 20G): "; DISK_SIZE="${DISK_SIZE:-20G}"; vm_validate_input "size" "$DISK_SIZE" && break; done
    while true; do vm_read MEMORY "Memory in MB (default: 2048): "; MEMORY="${MEMORY:-2048}"; vm_validate_input "number" "$MEMORY" && break; done
    while true; do vm_read CPUS "Number of CPUs (default: 2): "; CPUS="${CPUS:-2}"; vm_validate_input "number" "$CPUS" && break; done
    while true; do
        vm_read SSH_PORT "SSH Port (default: 2222): "
        SSH_PORT="${SSH_PORT:-2222}"
        if vm_validate_input "port" "$SSH_PORT" && ! ss -tln 2>/dev/null | grep -q ":$SSH_PORT "; then
            break
    ...
    local gui_input
    while true; do
        vm_read_char gui_input "Enable GUI mode? (y/n, default: n): "
        GUI_MODE=false
        gui_input="${gui_input:-n}"
        if [[ "$gui_input" =~ ^[Yy]$ ]]; then GUI_MODE=true; break; fi
        if [[ "$gui_input" =~ ^[Nn]$ ]]; then break; fi
    ...
    if vm_setup_image; then
        vm_save_config
        vm_print_status SUCCESS "VM '$VM_NAME' created successfully."
        vm_print_status INFO "Login with: username=$USERNAME, password=$PASSWORD"
...
vm_start() {
    ...
    if vm_load_config "$vm_name"; then
        ...
        if vm_is_running "$vm_name"; then
            vm_print_status WARN "VM '$vm_name' is already running"
            local restart_choice
            vm_read_char restart_choice "Stop and restart? (y/N): "
            if [[ "$restart_choice" =~ ^[Yy]$ ]]; then
                vm_stop "$vm_name" && sleep 2
            else
                return 1
            fi
        fi
        ...
        if [ ! -f "$IMG_FILE" ] || [ ! -f "$SEED_FILE" ]; then
            vm_print_status WARN "Image or seed file not found, recreating..."
        ...
        if [ "$GUI_MODE" = true ]; then
            ...
            vm_print_status INFO "Starting in console mode... (Ctrl+A, X to exit)"
        fi
        qemu_cmd+=(-device virtio-balloon-pci -object rng-random,filename=/dev/urandom,id=rng0 -device virtio-rng-pci,rng=rng0)
        vm_print_status INFO "Configuration: ${MEMORY}MB RAM, ${CPUS} CPUs, ${DISK_SIZE} disk"
        if ! "${qemu_cmd[@]}"; then
            vm_print_status ERROR "Failed to start VM."
            rm -f "${IMG_FILE}.lock" 2>/dev/null
            return 1
        fi
        vm_print_status INFO "VM $vm_name has been shut down"
    fi
}
...
vm_is_running() {
    ...
    if vm_load_config "$vm_name" 2>/dev/null && pgrep -f "qemu-system.*$IMG_FILE" >/dev/null; then
        return 0
    fi
    return 1
}
...
vm_stop() {
    ...
    if vm_load_config "$vm_name"; then
        if vm_is_running "$vm_name"; then
            ...
            if vm_is_running "$vm_name"; then
                vm_print_status WARN "VM did not stop gracefully, forcing..."
                pkill -9 -f "qemu-system.*$IMG_FILE" && sleep 1
            fi
            rm -f "${IMG_FILE}.lock" 2>/dev/null
            ! vm_is_running "$vm_name" && vm_print_status SUCCESS "VM $vm_name stopped" || vm_print_status ERROR "Failed to stop VM"
        else
...
vm_delete() {
    ...
    if [[ "$reply" =~ ^[Yy]$ ]]; then
        if vm_load_config "$vm_name"; then
            vm_is_running "$vm_name" && vm_print_status WARN "VM is running. Stopping it first..." && vm_stop "$vm_name" && sleep 2
...
vm_edit_config() {
    ...
    if vm_load_config "$vm_name"; then
        while true; do
            ...
            case $edit_choice in
                1) local new_hostname; vm_read new_hostname "New hostname (current: $HOSTNAME): "; HOSTNAME="${new_hostname:-$HOSTNAME}"; config_changed=true ;;
                2) local new_username; vm_read new_username "New username (current: $USERNAME): "; USERNAME="${new_username:-$USERNAME}"; config_changed=true ;;
                3) local new_password; vm_read_secret new_password "New password: "; PASSWORD="${new_password:-$PASSWORD}"; config_changed=true ;;
                4) local new_ssh_port; vm_read new_ssh_port "New SSH port (current: $SSH_PORT): "; SSH_PORT="${new_ssh_port:-$SSH_PORT}" ;;
                5) local gui_input; vm_read_char gui_input "Enable GUI mode? (y/n, current: $GUI_MODE): "; if [[ "$gui_input" =~ ^[Yy]$ ]]; then GUI_MODE=true; elif [[ "$gui_input" =~ ^[Nn]$ ]]; then GUI_MODE=false; fi ;;
                6) local new_port_forwards; vm_read new_port_forwards "Port forwards (current: ${PORT_FORWARDS:-None}): "; PORT_FORWARDS="${new_port_forwards:-$PORT_FORWARDS}" ;;
                7) local new_memory; vm_read new_memory "New memory in MB (current: $MEMORY): "; MEMORY="${new_memory:-$MEMORY}" ;;
                8) local new_cpus; vm_read new_cpus "New CPU count (current: $CPUS): "; CPUS="${new_cpus:-$CPUS}" ;;
                9) vm_print_status ERROR "Disk resize must be done from the main menu (option 7)." ;;
                0) break ;;
                *) vm_print_status ERROR "Invalid selection" ;;
            esac
            if [ "$config_changed" = true ]; then
                vm_print_status INFO "Updating cloud-init configuration..."
                vm_setup_image
            fi
            vm_save_config
            local continue_editing
            vm_read_char continue_editing "Continue editing? (y/N): "
...
vm_resize_disk() {
    ...
    if vm_load_config "$vm_name"; then
        if vm_is_running "$vm_name"; then
            vm_print_status ERROR "Cannot resize disk while VM is running."
            return 1
        fi
        vm_print_status INFO "Current disk size: $DISK_SIZE"
        local new_disk_size
        while true; do
            vm_read new_disk_size "Enter new disk size (e.g., 50G): "
            if vm_validate_input "size" "$new_disk_size"; then
                if [ "$new_disk_size" == "$DISK_SIZE" ]; then
                    vm_print_status INFO "New disk size is the same. No changes made."
                    return 0
                fi
                local current_size_gb; current_size_gb=$(echo "$DISK_SIZE" | sed 's/G//' | sed 's/g//')
                local new_size_gb; new_size_gb=$(echo "$new_disk_size" | sed 's/G//' | sed 's/g//')
                if [ "$new_size_gb" -lt "$current_size_gb" ]; then
                    vm_print_status WARN "Shrinking disk is not recommended and may cause data loss!"
                    local confirm_shrink
                    vm_read_char confirm_shrink "Are you sure? (y/N): "
                    if [[ ! "$confirm_shrink" =~ ^[Yy]$ ]]; then
                        vm_print_status INFO "Disk resize cancelled."
                        return 0
                    fi
                fi
...
vm_performance() {
    ...
    if vm_load_config "$vm_name"; then
        if vm_is_running "$vm_name"; then
            ...
            local qemu_pid; qemu_pid=$(pgrep -f "qemu-system.*$IMG_FILE")
            if [ -n "$qemu_pid" ]; then
                ...
                vm_print_status ERROR "Could not find QEMU process for VM $vm_name"
            fi
        else
            vm_print_status INFO "VM $vm_name is not running"
        fi
    ...
    while true; do
        ...
        case $choice in
            ...
            2)
                if [ $vm_count -gt 0 ]; then
                    local vm_num
                    vm_read vm_num "VM number to start: "
                    if [[ "$vm_num" =~ ^[0-9]+$ ]] && [ "$vm_num" -ge 1 ] && [ "$vm_num" -le $vm_count ]; then
                        vm_start "${vms[$((vm_num-1))]}"
                    else
                        vm_print_status ERROR "Invalid selection"
                    fi
                fi
            ...
            3)
                if [ $vm_count -gt 0 ]; then
                    local vm_num
                    vm_read vm_num "VM number to stop: "
                    if [[ "$vm_num" =~ ^[0-9]+$ ]] && [ "$vm_num" -ge 1 ] && [ "$vm_num" -le $vm_count ]; then
                        vm_stop "${vms[$((vm_num-1))]}"
                    else
                        vm_print_status ERROR "Invalid selection"
                    fi
                fi
            ...
            4)
                if [ $vm_count -gt 0 ]; then
                    local vm_num
                    vm_read vm_num "VM number to show info: "
                    if [[ "$vm_num" =~ ^[0-9]+$ ]] && [ "$vm_num" -ge 1 ] && [ "$vm_num" -le $vm_count ]; then
                        vm_show_info "${vms[$((vm_num-1))]}"
                    else
                        vm_print_status ERROR "Invalid selection"
                    fi
                fi
            ...
            5)
                if [ $vm_count -gt 0 ]; then
                    local vm_num
                    vm_read vm_num "VM number to edit: "
                    if [[ "$vm_num" =~ ^[0-9]+$ ]] && [ "$vm_num" -ge 1 ] && [ "$vm_num" -le $vm_count ]; then
                        vm_edit_config "${vms[$((vm_num-1))]}"
                    else
                        vm_print_status ERROR "Invalid selection"
                    fi
                fi
            ...
            6)
                if [ $vm_count -gt 0 ]; then
                    local vm_num
                    vm_read vm_num "VM number to delete: "
                    if [[ "$vm_num" =~ ^[0-9]+$ ]] && [ "$vm_num" -ge 1 ] && [ "$vm_num" -le $vm_count ]; then
                        vm_delete "${vms[$((vm_num-1))]}"
                    else
                        vm_print_status ERROR "Invalid selection"
                    fi
                fi
            ...
            7)
                if [ $vm_count -gt 0 ]; then
                    local vm_num
                    vm_read vm_num "VM number to resize disk: "
                    if [[ "$vm_num" =~ ^[0-9]+$ ]] && [ "$vm_num" -ge 1 ] && [ "$vm_num" -le $vm_count ]; then
                        vm_resize_disk "${vms[$((vm_num-1))]}"
                    else
                        vm_print_status ERROR "Invalid selection"
                    fi
                fi
            ...
            8)
                if [ $vm_count -gt 0 ]; then
                    local vm_num
                    vm_read vm_num "VM number to show performance: "
                    if [[ "$vm_num" =~ ^[0-9]+$ ]] && [ "$vm_num" -ge 1 ] && [ "$vm_num" -le $vm_count ]; then
                        vm_performance "${vms[$((vm_num-1))]}"
                    else
                        vm_print_status ERROR "Invalid selection"
                    fi
                fi
            ...
            9)
                if [ $vm_count -gt 0 ]; then
                    local vm_num
                    vm_read vm_num "VM number to fix issues: "
                    if [[ "$vm_num" =~ ^[0-9]+$ ]] && [ "$vm_num" -ge 1 ] && [ "$vm_num" -le $vm_count ]; then
                        vm_fix_issues "${vms[$((vm_num-1))]}"
                    else
                        vm_print_status ERROR "Invalid selection"
                    fi
                fi
